...
folder are mapped to the application's root namespace, 'SafetyPlatform'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SafetyPlatform"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SafetyPlatform' in the same way
as `"SafetyPlatform/sass/src"`.
